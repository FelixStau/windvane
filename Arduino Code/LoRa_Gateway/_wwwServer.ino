...
void buttonDocu() 
{
	...
	response += "      document.location.href = \"https://things4u.github.io/Projects/SingleChannelGateway/UserGuide/1_Introduction.html\"; ";
	response += "    }";
	response += "  }";
	response += "}";
...
static void openWebPage()
{
	...
	if (gwayConfig.refresh) {
		...
		response += "'><TITLE>LoRa 1ch Gateway</TITLE>";
	}
	else {
		response += String() + "<!DOCTYPE HTML><HTML><HEAD><TITLE>LoRa 1ch Gateway</TITLE>";
	}
#else
	response += String() + "<!DOCTYPE HTML><HTML><HEAD><TITLE>LoRa 1ch Gateway</TITLE>";
#endif
...
/*   
   ...
   response += ("background: -moz-linear-gradient(");
   response += ("top,");
   response += ("#ffffff 0%,");
   response += ("#ffffff 50%,");
   response += ("#bdbbbd);");
   response += ("background: -webkit-gradient(");
   response += ("linear, left top, left bottom,");
   response += ("from(#ffffff),");
   response += ("color-stop(0.50, #ffffff),");
   response += ("to(#bdbbbd));");
   response += ("-moz-border-radius: 10px;");
   response += ("-webkit-border-radius: 10px;");
   response += ("border-radius: 10px;");
   response += ("border: 3px solid #dedcd5;");
   response += ("-moz-box-shadow:");
   response += ("0px 1px 3px rgba(000,000,000,0.5),");
   response += ("inset 0px 0px 3px rgba(255,255,255,1);");
   response += ("-webkit-box-shadow:");
   response += ("0px 1px 3px rgba(000,000,000,0.5),");
   response += ("inset 0px 0px 3px rgba(255,255,255,1);");
   response += ("box-shadow:");
   response += ("0px 1px 3px rgba(000,000,000,0.5),");
   ...
   response += ("0px -1px 0px rgba(000,000,000,0.1),");
   response += ("0px 1px 0px rgba(255,255,255,1);");
   response += ("}");
*/   
   // Text color definitions for heat letters
   response += ("a: {color: rgb(255,255,255);}");
   response += ("h1 {color: rgb(255,255,255);}");
   response += ("h2 {color: rgb(255,255,255);}");
   response += ("h3 {color: rgb(255,255,255);}");
   response += ("h4 {color: rgb(255,255,255);}");
   response += ("h5 {color: rgb(255,255,255);}");
   response += ("h6 {color: rgb(255,255,255);}");
   response += ("h7 {color: rgb(255,255,255);}");
   ...
   // Background definitions carbon grid
   response += ("body {");
   response += ("background-color: rgb(32, 32, 32);");
   response += ("background-image: linear-gradient(45deg, black 25%, transparent 25%, transparent 75%, black 75%, black),");
   response += ("linear-gradient(45deg, black 25%, transparent 25%, transparent 75%, black 75%, black),");
   response += ("linear-gradient(to bottom, rgb(8, 8, 8), rgb(32, 32, 32));");
   response += ("background-size: 10px 10px, 10px 10px, 10px 5px;");
   response += ("background-position: 0px 0px, 5px 5px, 0px 0px;");
   response += ("}");
   
   response += ("</style>");


	response += "<style>.thead {background-color:#797d7f; color:white;} ";
	response += ".cell {border: 1px solid #797d7f;}";
	response += ".config_table {max_width:100%; min-width:400px; width:98%; border:1px solid black; border-collapse:collapse;}";
	response += "</style></HEAD><BODY>";
	
	response +="<h1>ESP LoRa Gateway Config</h1>";

	response +="<p style='font-size:12px;'>";
...
static void settingsData() 
{
	...
	response +="<th colspan=\"2\" style=\"background-color: #797d7f; color: white; width:120px;\">Value</th>";
	response +="<th colspan=\"4\" style=\"background-color: #797d7f; color: white; width:100px;\">Set</th>";
	response +="</tr>";
	
	bg = " background-color: ";
	bg += ( _cad ? "green" : "red" );
	response +="<tr><td class=\"cell\">CAD</td>";
	response +="<td colspan=\"2\" style=\"border: 1px solid #797d7f;"; response += bg; response += "\">";
	response += ( _cad ? "ON" : "OFF" );
	response +="<td style=\"border: 1px solid #797d7f; width:40px;\"><a href=\"CAD=1\"><button>ON</button></a></td>";
	response +="<td style=\"border: 1px solid #797d7f; width:40px;\"><a href=\"CAD=0\"><button>OFF</button></a></td>";
	response +="</tr>";
	...
	response +="<td style=\"border: 1px solid #797d7f; width:40px;\"><a href=\"HOP=1\"><button>ON</button></a></td>";
	response +="<td style=\"border: 1px solid #797d7f; width:40px;\"><a href=\"HOP=0\"><button>OFF</button></a></td>";
	response +="</tr>";
	...
	response +="<td class=\"cell\" colspan=\"2\">";
	response +=frameCount;
	response +="</td><td colspan=\"2\" style=\"border: 1px solid #797d7f;\">";
	response +="<button><a href=\"/FCNT\">RESET</a></button></td>";
	response +="</tr>";
	
	bg = " background-color: ";
	bg += ( (gwayConfig.isNode == 1) ? "green" : "red" );
	response +="<tr><td class=\"cell\">Gateway Node</td>";
	response +="<td class=\"cell\" style=\"border: 1px solid #797d7f;" + bg + "\">";
	response += ( (gwayConfig.isNode == true) ? "ON" : "OFF" );
	response +="<td style=\"border: 1px solid #797d7f; width:40px;\"><a href=\"NODE=1\"><button>ON</button></a></td>";
	response +="<td style=\"border: 1px solid #797d7f; width:40px;\"><a href=\"NODE=0\"><button>OFF</button></a></td>";
	response +="</tr>";
#endif

#if A_REFRESH==1
	bg = " background-color: ";
	bg += ( (gwayConfig.refresh == 1) ? "green" : "red" );
	response +="<tr><td class=\"cell\">WWW Refresh</td>";
	response +="<td class=\"cell\" colspan=\"2\" style=\"border: 1px solid #797d7f; " + bg + "\">";
	response += ( (gwayConfig.refresh == 1) ? "ON" : "OFF" );
	response +="<td style=\"border: 1px solid #797d7f; width:40px;\"><a href=\"REFR=1\"><button>ON</button></a></td>";
	response +="<td style=\"border: 1px solid #797d7f; width:40px;\"><a href=\"REFR=0\"><button>OFF</button></a></td>";
	response +="</tr>";
#endif
...
static void systemData()
{
	if (gwayConfig.expert) {
		...
		response +="<tr><td style=\"border: 1px solid #797d7f; width:120px;\">Gateway ID</td>";
...
// XXX We Shoudl find an ESP32 alternative
		...
		response +="<td style=\"border: 1px solid #797d7f; width:40px;\"><a href=\"SPEED=80\"><button>80</button></a></td>";
		response +="<td style=\"border: 1px solid #797d7f; width:40px;\"><a href=\"SPEED=160\"><button>160</button></a></td>";
		response+="</tr>";
		response +="<tr><td class=\"cell\">ESP Chip ID</td><td class=\"cell\">"; response+=ESP.getChipId(); response+="</tr>";
#else
    uint64_t macAddress = ESP.getEfuseMac();
    uint64_t macAddressTrunc = macAddress << 40;
    uint chipID = macAddressTrunc >> 40;
    response +="<tr><td class=\"cell\">ESP Chip ID</td><td class=\"cell\">"; response+=chipID; response+="</tr>";
#endif
...
static void interruptData()
{
	if (gwayConfig.expert) {
		...
		response +="</td>";
		response +="<td colspan=\"2\" style=\"border: 1px solid #797d7f;\">";
